match.group <- function(b,n.group)
{
  #a=c(1:20,41,42,45,46,49,50,53,54,57,58,83,84,87,88,91,92,95,96,99,100)
  a=c(1:8,9,14,19,24,25,30,35,40)
...
sim <- function(num,n.group,g.size,matched=1,quan,g,mag,k,perc,replic,epsilon,n.step,gamma,example)
{
  ...
  for ( i in 1:replic)
  {
    ...
    outfile1 <- gsub("( )", "", paste("/space/jliu20/Compound_bridge/Sim/group_selected_gMCP_",num,"_",k,"_",perc,"_",mag,"_",matched,"_",gamma,"_",example,".txt"))
  ...
  outfile2 <- gsub("( )", "", paste("/space/jliu20/Compound_bridge/Sim/sse_surv_gMCP_",num,"_",k,"_",perc,"_",mag,"_",matched,"_",gamma,"_",example,".txt"))
...
sim.outer <- function(num,n.group,g.size,quan,g,mag,k,perc,replic,epsilon,n.step,gamma,example)
{
  tp <- numeric();tn <- numeric();fp <- numeric();fn <- numeric();sse.seq <- numeric();
  ...
  for ( i in 1:replic)
  {
    ...
    beta2 <- c(rep(0.15,g.size),rep(0.1,g.size),rep(0,g.size),rep(0.15,g.size),rep(0,2*g.size),rep(0.1,g.size),rep(0,g.size*(n.group-7)));
    beta3 <- c(rep(0.1,g.size),rep(0.15,g.size),rep(0,2*g.size),rep(0.15,g.size),rep(0,2*g.size),rep(0.1,g.size),rep(0,g.size*(n.group-8)));
    #beta4 <- c(rep(0.1,g.size),rep(0.15,g.size),rep(0,3*g.size),rep(0.15,g.size),rep(0,3*g.size),rep(0.1,g.size),rep(0,g.size*(n.group-10)));
    #group: 1:8,9,14,19,24,25,30,35,40
    ...
    pos_e <- cumsum(group.outer); pos_s <- pos_e -group.outer + 1;
    pos_e_in <- cumsum(group.inner); pos_s_in <- pos_e_in -group.inner + 1;
    group.o <- numeric(length(group.outer))
    for ( j in 1:length(group.outer))
      group.o[j] <- pos_e_in[pos_e[j]] - pos_s_in[pos_s[j]] +1 ;

    #source("/space/jliu20/Compound_bridge/cv_mult_gMCP.r");n.fold <- 5; 
    #cv <- cv.optim(cl,x,y,n.t,group.o,n.fold,epsilon,n.step,gamma)
    #PE <- cv$PE; lambda.seq <- cv$lambda.seq; lambda.opt <- lambda.seq[PE==min(PE)]
    source("/space/jliu20/Compound_bridge/BIC.gMCP.r")
    BIC.t <-  BIC.gMCP(x,y,n.t,group.o,epsilon,n.step,gamma,g)
    ...
    fm <- GMCP_quant(x,y,group.o,lambda.opt,gamma)
    sel.group <- identify.group(fm$beta,group.inner)$ident.g;
    if ( is.null(sel.group) == T) sel.group <- NA; 
    sse <- sum((y-x%*%fm$beta)^2)
    sse.seq <- c(sse.seq, sse)  
                                                                   
    outfile1 <- gsub("( )", "", paste("/space/jliu20/Compound_bridge/Sim/group_selected_gMCP_outer_",num,"_",k,"_",perc,"_",mag,"_",matched,"_",gamma,"_",example,".txt"))
    write.table(matrix(sel.group,nrow=1),outfile1,sep=" ",quote=F, row.names=F,col.names=F,append=T);
    
    a <- match.group(sel.group,n.group*4)
    tp = c(tp,a$tp);tn = c(tn,a$tn);fp = c(fp,a$fp);fn = c(fn,a$fn)
  }
  
  outfile2 <- gsub("( )", "", paste("/space/jliu20/Compound_bridge/Sim/sse_surv_gMCP_outer_",num,"_",k,"_",perc,"_",mag,"_",matched,"_",gamma,"_",example,".txt"))
  write.table(sse.seq,outfile2,sep=",",quote=F, row.names=F,col.names=F,append=T);
...
#source("/space/jliu20/Compound_bridge/sim_gMCP.bic.r")
#source("/space/jliu20/Compound_bridge/generate.surv.categorical.r")
#sim <- function(num,n.group,g.size,quan,g,mag,k,perc,replic,epsilon,n.step,gamma,example)
#sim <- function(num,n.group,g.size,matched=1,quan,g,mag,k,perc,replic,epsilon,n.step,gamma,example)
#num=100;n.group=200;g.size=5;matched=1;quan=0.9;g=0;mag=0.5;k=0.1;perc=0.9;epsilon=0.05;n.step=40;gamma=10000;
# 25%
#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=0.2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR02") #node01
#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=0.5,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR05") #node01
#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=0.8,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR08") #node01

#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=1,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node02
#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node02
#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=3,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node02

#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=0.2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"AR02") #node03
#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=0.5,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"AR05") #node03
#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=0.8,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"AR08") #node03

#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=1,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"Band") #node04
#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"Band") #node04
#s <- sim(100,200,5,matched=2,quan=0.9,g=0,mag=3,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"Band") #node04

#source("/space/jliu20/Compound_bridge/sim_gMCP.bic.r")
#source("/space/jliu20/Compound_bridge/generate.surv.categorical.r")
# 0%
#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=0.2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR02") #node06
#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=0.5,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR05") #node06
#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=0.8,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR08") #node06

#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=1,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node07
#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node07
#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=3,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node07

#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=0.2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"AR02") #node08
#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=0.5,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"AR05") #node08
#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=0.8,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"AR08") #node08

#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=1,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"Band") #node09
#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"Band") #node09
#s <- sim(100,200,5,matched=3,quan=0.9,g=0,mag=3,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=6,"Band") #node09

#source("/space/jliu20/Compound_bridge/sim_gMCP.bic.r")
#source("/space/jliu20/Compound_bridge/generate.surv.categorical.r")
#matched=4; group Lasso
#s <- sim(100,200,5,matched=4,quan=0.9,g=0,mag=0.2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR02") #node01
#s <- sim(100,200,5,matched=4,quan=0.9,g=0,mag=0.5,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR05") #node01
#s <- sim(100,200,5,matched=4,quan=0.9,g=0,mag=0.8,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR08") #node01

#s <- sim(100,200,5,matched=4,quan=0.9,g=0,mag=1,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node02
#s <- sim(100,200,5,matched=4,quan=0.9,g=0,mag=2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node02
#s <- sim(100,200,5,matched=4,quan=0.9,g=0,mag=3,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node02

#matched=5; group Lasso
#s <- sim(100,200,5,matched=5,quan=0.9,g=0,mag=0.2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR02") #node03
#s <- sim(100,200,5,matched=5,quan=0.9,g=0,mag=0.5,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR05") #node03
#s <- sim(100,200,5,matched=5,quan=0.9,g=0,mag=0.8,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR08") #node03

#s <- sim(100,200,5,matched=5,quan=0.9,g=0,mag=1,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node04
#s <- sim(100,200,5,matched=5,quan=0.9,g=0,mag=2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node04
#s <- sim(100,200,5,matched=5,quan=0.9,g=0,mag=3,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node04

#matched=6; group Lasso
#s <- sim(100,200,5,matched=6,quan=0.9,g=0,mag=0.2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR02") #node06
#s <- sim(100,200,5,matched=6,quan=0.9,g=0,mag=0.5,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR05") #node06
#s <- sim(100,200,5,matched=6,quan=0.9,g=0,mag=0.8,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"AR08") #node06

#s <- sim(100,200,5,matched=6,quan=0.9,g=0,mag=1,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node07
#s <- sim(100,200,5,matched=6,quan=0.9,g=0,mag=2,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node07
#s <- sim(100,200,5,matched=6,quan=0.9,g=0,mag=3,0.1,0.9,5,epsilon=0.01,n.step=40,gamma=10000,"Band") #node07
